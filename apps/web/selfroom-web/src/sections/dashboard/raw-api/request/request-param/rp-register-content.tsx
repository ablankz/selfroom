...
export default function RPRegisterContent({
  param,
  onClose,
  setFilters,
  filters,
    ...
    key: Yup.string()
      ...
      .test(
        ...
        function (this: Yup.TestContext, data: string) {
          return !(
            newCreate && Object.keys(filters.param).some((e) => e === data)
          );
        }
      ),
  ...
  const onSubmit = async (data: Param) => {
    ...
    reset();
    enqueueSnackbar(param ? t('Updated PARAM values') : t('Add PARAM value'));
    onClose();
  };
  ...
  return (
    <DialogContent>
      <DialogContentText>
        {t('Please enter the PARAM information you wish to add')}
      </DialogContentText>
